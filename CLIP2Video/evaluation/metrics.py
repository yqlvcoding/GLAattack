...
def compute_metrics(x):
    tot_video_num = x.shape[0]
    sx = np.sort(-x, axis=1)
    d = np.diag(-x)
    d = d[:, np.newaxis]
    ind = sx - d
    
    ind = (ind==0).argmax(axis=1)
    #ind = np.where(ind == 0)
    #ind = ind[1]
    
    metrics = {}
    metrics['R1'] = float(np.sum(ind == 0)) * 100 / tot_video_num
    metrics['R5'] = float(np.sum(ind < 5)) * 100 / tot_video_num
    metrics['R10'] = float(np.sum(ind < 10)) * 100 / tot_video_num
...
def tensor_text_to_video_metrics(sim_tensor, top_k = [1,5,10]):
    ...
    stacked_sim_matrices = sim_tensor.permute(1, 0, 2)   # 81 * 670 * 670
