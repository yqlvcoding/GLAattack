...
class Mydataset(data.Dataset):
    def __init__(self, data_path, num_frame, downstream_task, transform=None):
    ...
    def _parse_list(self):
        ...
        tmp.sort()
        self.video_list = tmp
        # if self.downstream_task == 'Actionclip':
        #     with open("Actionclip/keyid.json", "r") as fp:
    ...
    def get(self, record, indices):
        ...
        for i, seg_ind in enumerate(indices):
            ...
            try:
                ...
                seg_imgs = self._load_image(os.path.join(self.data_path, record, str(p) + ".png"))
                #print(np.asarray(seg_imgs[0], dtype=np.int).shape)
            except OSError:
                print('ERROR: Could not read image "{}"'.format(os.path.join(self.data_path, record, str(p) + ".png / .jpg")))
