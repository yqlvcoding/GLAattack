...
class RawFrameExtractor():
    ...
    def _transform(self, n_px):
        return Compose([
            # Resize(n_px, interpolation=Image.BICUBIC),   # as already pre-solved
            # CenterCrop(n_px),
            ToTensor(),
    ...
    def video_to_tensor(self, video_file, max_frame, preprocess, sample_fp=0, myreplace=None):
        ...
        if max_frame >=  current_sample_indx.shape[0]:
            ...
            print("################ something error as pre_solve frame ###################")
        ...
        # ### here my code
        # video_now_n = video_file.split('/')[-1]
        # target_path = os.path.join('/share/test/lyq/GLAattack/clip2video/correct', video_now_n)
        ...
        for index in frame_index:
            if myreplace:
                image_path = os.path.join(myreplace, video_name[current_sample_indx[int(index)]].replace(".jpg", ".png"))
            else:
                image_path = os.path.join(video_file, video_name[current_sample_indx[int(index)]])
            
            # #### here my code
            # target_file = os.path.join(target_path, video_name[current_sample_indx[int(index)]].replace(".jpg", ".png"))
    ...
    def get_video_data(self, video_path, max_frame, myreplace=None):
        """get video data
        Args:
            video_path: id
            max_frame: max frame number
        ...
        image_input = self.video_to_tensor(video_path, max_frame, self.transform, sample_fp=self.framerate, myreplace=myreplace)
