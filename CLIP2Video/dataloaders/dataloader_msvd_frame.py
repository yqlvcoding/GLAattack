...
class MSVD_multi_sentence_dataLoader(Dataset):
    ...
    def __init__(
        ...
        video_id_path_dict["test"] = os.path.join(self.data_path, "correct_test_list.txt")
    ...
    def _get_rawvideo(self, video_id):
        ...
        if self.myreplace:
            myreplace = os.path.join(self.myreplace, video_id)
        else:
            myreplace = None
        # get sampling frames
        raw_video_data = self.frameExtractor.get_video_data(video_path, self.max_frames, myreplace=myreplace)
        raw_video_data = raw_video_data['video']  # max, 3, 224, 224
